            ...
            {Array.isArray(nodes) && nodes.map((node, index) => {
                ...
                return (
                    <AccordionItem key={id} value={id}>
                        ...
                        <AccordionContent>
                            <div
                                className={`pl-4 py-1 cursor-pointer rounded text-lg ${selected === node.name ? "bg-primary text-white" : "hover:bg-muted"
                                    }`}
                                onClick={() => handleProgramSelect(node)}
                            >
                                🎓 {node.name}
                            </div>
                            {/* Use depth prop to control recursion */}
                            {level < depth - 1 && Array.isArray(node.children) && node.children.length > 0 && (
                                <ProgramAccordion<T>
                                    nodes={node.children}
                                    level={level + 1}
                                    depth={depth} // ← This was missing! Pass depth down
                                    setValue={setValue}
                                    fieldKey={fieldKey}
...
interface ProgramAccordionDisplayProps<T extends genericObjectType> {
    ...
    heading?: string;
    subHeading?: string;
    depth?: number; // Add depth prop here too
}
export const ProgramAccordionDisplay = <T extends genericObjectType>({
    ...
    return (
        <div className="max-w-3xl mx-auto p-4">
            ...
            <ProgramAccordion
                nodes={programs}
                depth={depth} // Use the depth prop
                setValue={setValue}
                watch={watch}
                fieldKey={fieldKey}
                fieldIdKey={fieldIdKey}
